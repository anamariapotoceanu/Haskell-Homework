...
-- Vrem sa aplicam functia Region asupra punctului
-- Pentru a face acest lucru, trebuie sa inversam ordinea
...
{-  
    Specificam prima data lista de puncte, apoi punctul pentru
    care facem verificarea, astfel folosim flip pentru a inversa ordinea
    argumentelor.
-}
...
rectangle :: Float -> Float -> Region
    --Verificam daca coordonatele x si y se afla in intervalul dreptunghiului.
...
circle :: Float -> Region
{-
    Ecuatia cercului de centru C(a, b) si raza r este:
    (x - a)^2 + (y - b)^2 <= r^2
-}
circle radius = \(x,y) ->
    x^2 + y^2 <= radius^2 
...
{-
    Se creeaza toate perechile de coordonate posibile, x incepe
    de la -width pana la width, iar y de la height pana la -height.
    Verificam daca apartin regiunii. In cazul in care apartin, vom afisa *,
    altfel ".".
-}
plot width height region =
    intercalate "\n" [
        [case inside (fromIntegral x, fromIntegral y) region of
            True -> '*'
            False -> '.'
          | x <- [-width, -width + 1 .. width]]
...
{-
    Pentru promoteUnary avem compunere de doua functii, unde prima functie
    se aplica rezulatului celei de-a doua functie. Prima functie este
    f : a->b, iar a doua functie este g : Point -> a (Pointed a). Rezulatul va
    fi h : Point->b (Pointed b).
-}
promoteUnary = (.)

promoteBinary :: (a -> b -> c) -> Pointed a -> Pointed b -> Pointed c
{-
    Avem doua functii: pointed1 si pointed2. Ambele se aplica asupra aceluiasi
    punct. Rezultatele vor fi argumentele functiei f.
-}
...
{-
    Pentru a realiza functia complement, vom folosi functia not, care
    inverseaza valorile. True devine False, iar False devine True.
-}
complement = promoteUnary not

union :: Region -> Region -> Region
{-
    Functia union are rolul de a returna regiunea formata din reuniunea
    a doua regiuni. Vom folosi operatorul "||" (sau). Un punct se afla in
    reuniune doar daca se regaseste in una dintre cele doua regiuni.
-}
union = promoteBinary (||)

intersection :: Region -> Region -> Region
{-
    Functia union are rolul de a returna regiunea formata din intersectia
    a doua regiuni. Vom folosi operatorul "&&" (si). Un punct se afla in
    intersectie doar daca se regaseste in ambele regiuni.
-}
...
{-
    Din coordonatele initiale scadem deplasamentele.
-}
...
{-
    Coordonatele initiale se impart la factorul primit ca parametru.
-}
...
{-
    Cum Tranformation este definita ca Point->Point, iar Region
    ca Point->Bool, avem nevoie sa realizam compunerea functiilor in ordine 
    inversa. Rezulatul este o functie Point->Bool. Astfel vom considera prima
    functie g : Point->Bool, a doua functie f : Point->Point. Realizand
    compunerea lor obtinem h : Point->Bool.
-}
...
{-
    Pentru ca prima transformare din lista sa fie aplicata prima, trebuie sa
    folosim flip. Utilizand flip, transformarile vor fi aplicate in ordinea
    in care apar, deoarece se interschimba ordinea compunerii functiilor.
    De exemplu daca avem doua transformari f si g. [f, g] trebuie sa obtinem
    g . f, astfel prima transformare care se realizeaza este f, prima din
    lista.
-}
...
{-
    *** TODO ***
    ...
    Răspuns: Functia de mai jos realizeaza o reuniune de cercuri, astfel primul
    cerc generat va fi cel centrat in origine, iar celelalte cercuri vor fi 
    translatate cu 6 unitati pe axa x. Utilizarea evaluarii lenese din Haskell
    este utila in cadrul acestei functii, deoarece nu se vor genera alte
    cercuri daca se constata faptul ca punctul primit ca parametru apartine
    unui cerc. Daca am gasit punctul intr-un cerc, evaluarea se opreste. 
    Nu mai construim si celelalte cercuri translatate.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: Functia de mai jos este o reuniune de mai multe cercuri. Prima
    data se creeaza primul cerc centrat in origine, de raza 2, apoi se vor
    genera o infinitate de cercuri translatate cu 6 unitati.
    Pentru un punct care nu apartine regiunii, prima data se verifica apartenta
    pentru primul cerc generat. Cum punctul nu se afla in primul cerc, procesul
    de verificare va continua pentru celelalte cercuri, deoarece mereu va
    exista un cerc translatat la stanga. Aceasta verificare se va realiza la
    infinit, deoarece va exista permanent un cerc pentru care se poate face
    verificarea.
-}
...
-- Vom incepe parcurgerea de la nodul de start
-- Vom retine cu ajutorul unui set ce noduri au fost vizitate
bfs start expand = bfs [(start, 0)] S.empty
  where
    -- Daca nu avem noduri de procesat, returnam lista vida
    [] = []
    -- Extragem nodul curent impreuna cu distanta corespunzatoare
    bfs ((node, d) : rest) visited =
    {- Daca nodul curent nu a fost vizitat, atunci adaugam nodul la lista
        si vom continua parcurgerea recursiv, apeland functia bfs
        In cazul in care nodul a mai fost vizitat, se continua evaluarea
        celorlalte noduri.
    -}
        if not (S.member node visited)
            then result
        else bfs rest visited
      where
        -- Nodul curent se adauga la lista si se continua parcurgerea
        -- Se marcheaza nodul ca fiind vizitat
        result = (node, d) : bfsRecursiv
        bfsRecursiv = bfs newListProcess newVisitedList
        {- La noua lista de procesat se adauga vecinii nodului pentru
            care se creste costul cu 1
            In cazul algoritmului BFS, trebuie sa procesam toti vecinii nodului
            Astfel, noua lista care va fi procesata va fi formata din restul
            nodurilor si din vecinii nodului curent pe care trebuie sa-i
            evaluam.
            De asemenea, nodul curent se marcheaza ca vizitat. 
        -}
        newListProcess = (rest ++ neigh)
        newVisitedList = S.insert node visited
        {-
            Vecinii nodului ii aflam cu ajutorul functiei expand. Pentru
            fiecare vecin se adauga 1 la costul curent/ distanta creste
            cu o unitate.
        -}
        neigh = map (\x -> (x, d + 1)) (expand node)
...
{-
    Functia regionAvoidingBfs primeste un punct de start si o regiune
    care trebuie evitata. 
    Pentru a face acest lucru, am folosit expand
    care va genera toti vecinii adiacenti ai unui punct. Vom alege
    doar punctele care nu se afla in interiorul regiunii date.

    Inside returneaza True daca punctul se afla in interiorul regiunii,
    astfel vom folosi "not" pentru a pastra doar punctele dorite.

    Cu ajutorul functiei bfs vom parcurge toate nodurile de la start
    si vom procesa fiecare nod in parte.
-}
regionAvoidingBfs start region = bfs start expand
    where
    expand (x, y) = filter (not . (`inside` region)) 
                    [(x + 1, y),
                     (x - 1, y), 
                     (x, y + 1),
                     (x, y - 1)] 